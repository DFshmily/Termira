...
- 终端顶部标签栏调整为接近 Termius 的深色胶囊式布局，仅保留会话标签和新标签入口，主机和 SFTP 入口分别交给左右侧工具栏承载。
...
- SFTP 文件列表改为紧凑文件管理器布局：路径和工具栏固定，文件列表独立纵向滚动并使用细暗色滚动条，传输队列固定在底部且多任务可独立滚动，空队列以轻量空闲态展示，长文件名保留可读空间。
