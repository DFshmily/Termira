...
- SFTP 文件列表在非根目录顶部展示 `..` 上级目录行，可像文件管理器一样直接返回父目录。
- SFTP 文件列表表头支持按名称、修改时间、大小、类型/权限排序，并显示当前排序方向。
- SFTP 修改时间统一展示完整年份，格式为 `YYYY/MM/DD HH:mm`。
- 右侧工具区宽度和 SFTP 传输队列高度支持拖拽调整，并保存到本地偏好。
- SFTP 支持拖拽上传：文件拖到列表区域上传到当前目录，拖到目录行上传到该目录；上传完成后自动刷新目标目录。
...
- 删除前使用确认框。
- 重命名使用列表内联编辑，确认后调用 `sftp.rename` 并刷新当前目录。
- 传输队列展示 queued、running、completed、failed、cancelled 状态，失败或取消后可重试。
- 终端输入简单 `cd` 命令时会同步当前标签 cwd，SFTP 面板在同一会话下自动跟随到新的远程目录。
...
- 打开 SFTP 面板后，远程用户目录下的 `.bash_history`、`.sudo_as_admin_successful` 等长名称能完整显示，列表区域可以单独纵向滚动且不出现横向滚动条。
...
- 终端输入简单 `cd /tmp`、`cd /`、`cd ..` 等命令时会更新当前标签 cwd，SFTP 面板会自动跟随到对应远程路径，工具栏仍保留手动“跳到终端当前目录”入口。
- 拖拽上传完成后会按任务目标目录刷新 SFTP 列表，避免队列显示完成但当前目录未出现新文件。
